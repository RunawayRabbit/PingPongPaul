...
public abstract class PortalBase : MonoBehaviour {
    ...
    private void CheckCorners(ref Vector3 offsetDirection, ref float distance, Vector3 centerPosition, Vector3 rightEndPosition, Vector3 leftEndPosition) {
        ...
        if (raycastRight == true) {
            //offsetDirection += (new Vector3(raycastRight.point.x, raycastRight.point.y) + transform.right) - (rightEndPosition + transform.right);
            offsetDirection += -transform.up;
        ...
        if (raycastLeft == true) {
            //offsetDirection += (new Vector3(raycastLeft.point.x, raycastLeft.point.y) + transform.right) - (leftEndPosition + transform.right);
            offsetDirection += transform.up;
